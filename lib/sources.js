...
function parseCarBlocksFromBody({
  ...
  for (const match of text.matchAll(rx)) {
    const block = cleanTitle(match[1]);
...
async function searchMercadoLibre(query, spec) {
  // Verified public SSR pages exist for model searches such as:
  // /usados/hyundai-tucson and /usados/toyota-rav4-usados.
  // The old MCR API can return 403, so we use the public results pages instead.
  const slug = slugify(
    [spec.brandLabel || spec.brand, spec.model].filter(Boolean).join(" ")
  );

  const candidates = [
    `https://listado.mercadolibre.co.cr/usados/${slug}`,
    `https://listado.mercadolibre.co.cr/usados/${slug}-usados`,
    `https://autos.mercadolibre.co.cr/autos-camionetas/usados/autos-usados-${slug}`,
  ];
  ...
  for (const url of candidates) {
    ...
    // First try individual links/cards.
    $("a[href]").each((_, el) => {
      ...
      rows.push(
        carOffer(
          ...
          "Mercado particular",
          {
            title: cleanCarTitle(text, Math.min(...prices)) || query,
            price: Math.min(...prices),
            url: absoluteUrl($(el).attr("href"), url),
            meta: text,
            condition: "Usado / consultar",
    ...
    // Fallback to the server-rendered text, which MercadoLibre exposes publicly.
    if (!rows.length) {
      ...
      rows.push(
        ...parseCarBlocksFromBody({
          text: body,
          source: "MercadoLibre",
          sourceGroup: "particular",
          sourceGroupLabel: "Mercado particular",
          url,
          query,
          spec,
          currency: "CRC",
    ...
    const clean = dedupeCarOffers(rows).slice(0, 35);
    if (clean.length) {
      return sourceStatus("MercadoLibre", url, clean);
    }

    lastError = "Sin coincidencias verificables";
  }
...
async function searchAutoCosmos(query, spec) {
  // Verified model pages are server-rendered, e.g.:
  // https://www.autocosmos.cr/auto/listado/toyota/rav4
  ...
  // Strong fallback: AutoCosmos exposes listings in the page body even when
  // the card HTML structure changes.
  if (!rows.length) {
    const body = cleanTitle($("body").text());
    rows.push(
      ...parseCarBlocksFromBody({
  ...
  return sourceStatus(
    ...
    clean.length
      ? null
      : spec.year
      ? `Sin anuncios ${spec.year} verificables`
      : "Sin coincidencias verificables"
  );
}

async function searchPurdy(query, spec, fx) {
  // Verified public inventory page currently exposes 100+ vehicles in USD.
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    $("a[href]").each((_, el) => {
      const box = findCardContainer($, el, 11);
      const text = cleanTitle(box.text());
    ...
    // Purdy's SSR page text is reliable even when card nesting changes.
    if (!rows.length) {
      const body = cleanTitle($("body").text());
      rows.push(
        ...parseCarBlocksFromBody({
  ...
  return sourceStatus(
    ...
    !successfulPages
      ...
      ? null
      : spec.year
      ? `Sin unidades ${spec.year} verificables`
      : "Sin coincidencias verificables"
  );
}
