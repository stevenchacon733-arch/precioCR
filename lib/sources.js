...
function walmartSearchPageUrl(query) {
  const q = encodeURIComponent(query.trim());
  return `https://www.walmart.co.cr/${q}?_q=${q}&map=ft`;
}

async function searchWalmartCatalog(config, query) {
  // Walmart Costa Rica exposes a public VTEX catalog search endpoint.
  // This is much more reliable than scraping the dynamically-rendered search HTML.
  const apiUrl =
    `https://www.walmart.co.cr/api/catalog_system/pub/products/search/` +
    `${encodeURIComponent(query.trim())}?_from=0&_to=49`;

  const sourceUrl = walmartSearchPageUrl(query);
  ...
  if (!fetched.ok || !Array.isArray(fetched.data)) {
    return {
      ...
      ok: false,
      count: 0,
      error: fetched.error || "Catálogo no disponible",
      offers: [],
    };
  }
  ...
  for (const product of fetched.data) {
    ...
    if (relevance(productName, query) < 0.5) continue;
    ...
    for (const item of items) {
      ...
      for (const seller of sellers) {
        ...
        const available =
          Number(offer?.AvailableQuantity ?? offer?.availableQuantity ?? 0);
        ...
        if (!Number.isFinite(price) || price < config.minPrice) continue;

        // Keep products that appear orderable; if availability is omitted, still keep it.
        if (Number.isFinite(available) && available <= 0 && "AvailableQuantity" in offer) {
          continue;
        }

        const itemName = cleanTitle(item?.nameComplete || item?.name || "");
        const title =
          itemName && relevance(itemName, query) >= 0.45
            ? itemName
            : productName;
        ...
        offers.push({
          ...
          url: absoluteUrl(link, "https://www.walmart.co.cr"),
          condition: "Nuevo",
          meta: extractMeta(`${productName} ${itemName}`, "tech"),
        });
      }
    }
  }

  const clean = dedupe(offers)
    .filter((x) => relevance(`${x.title} ${x.meta}`, query) >= 0.45)
  ...
  return {
    ...
    ok: true,
    count: clean.length,
    error: clean.length ? null : "Sin coincidencias verificables",
    offers: clean,
  };
}
...
async function searchOne(config, query, type) {
  if (config.domain === "walmart.co.cr") {
    return searchWalmartCatalog(config, query);
  }
