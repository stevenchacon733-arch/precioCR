...
const TECH_CONFIG = [
  ...
  },
  {
    ...
    minPrice: 5000,
    fallbackUrls: ["https://extremetechcr.com/"],
    buildUrl: (q) =>
      `https://extremetechcr.com/?s=${encodeURIComponent(q)}&post_type=product`,
  },
  {
    name: "Monge",
    domain: "tiendamonge.com",
    minPrice: 5000,
    fallbackUrls: ["https://www.tiendamonge.com/"],
    buildUrl: (q) =>
      `https://www.tiendamonge.com/catalogsearch/result/?q=${encodeURIComponent(q)}`,
  },
  {
    name: "MExpress",
    domain: "tiendamexpress.com",
    minPrice: 5000,
    fallbackUrls: ["https://www.tiendamexpress.com/tecnologia"],
    buildUrl: (q) =>
      `https://www.tiendamexpress.com/catalogsearch/result/?q=${encodeURIComponent(q)}`,
  },
  {
    name: "Verdugo",
    domain: "verdugotienda.com",
    minPrice: 5000,
    fallbackUrls: ["https://www.verdugotienda.com/"],
    buildUrl: (q) =>
      `https://www.verdugotienda.com/catalogsearch/result/?q=${encodeURIComponent(q)}`,
  },
];
...
function normalizeTechSearch(value = "") {
  return normalizeText(value)
    .replace(/\bplay\s+station\b/g, "playstation")
    .replace(/\bplay\s*station\b/g, "playstation")
    .replace(/\biphone\s+(\d+)/g, "iphone$1")
    .replace(/\bgalaxy\s+(s|a|z)\s*(\d+)/g, "galaxy $1$2")
    .replace(/\bps\s*(\d+)/g, "playstation $1")
    .replace(/\s+/g, " ")
    .trim();
}

function techSearchTokens(value = "") {
  const normalized = normalizeTechSearch(value);
...
function relevance(text, query) {
  const normalizedQuery = normalizeTechSearch(query);
  const tokens = techSearchTokens(normalizedQuery);
  if (!tokens.length) return 0;
  const haystack = normalizeTechSearch(text);
...
function parseAgencyInventoryPreview({ text, source, url, fx, limit = 12 }) {
  if (!fx?.rate) return [];
  const rows = [];
  const rx = /(\$\s*[\d.,]+)([\s\S]{0,420}?)(?=\$\s*[\d.,]+|$)/gi;

  for (const match of text.matchAll(rx)) {
    const block = normalizeBodyText(`${match[1]} ${match[2]}`);
    const originalPrice = parseUSDPrices(block)[0];
    const year = block.match(/\b(?:19|20)\d{2}\b/)?.[0];
    const price = usdToCrc(originalPrice, fx);
    if (!year || !price || price < 500000) continue;
    const title = cleanTitle(block, null);
    if (!/[a-z]{3,}/i.test(title)) continue;
    ...
    rows.push(carOffer(
      ...
      "Agencia / seminuevos",
      {
        ...
        meta: block,
        year: Number(year),
        condition: "Inventario de agencia / consultar",
      },
      { brand: null, modelTokens: [], year: null }
    ));

    if (rows.length >= limit) break;
  }

  return dedupeCarOffers(rows);
}
...
async function searchPurdy(query, spec, fx) {
  ...
  const preview = clean.length ? [] : pages
    .filter((page) => page.ok)
    .flatMap((page) => parseAgencyInventoryPreview({
      text: normalizeBodyText(cheerio.load(page.text)("body").text()),
      source: "Purdy Usados",
      url: base,
      fx,
    }))
...
async function searchGrupoQ(query, spec, fx) {
  ...
  const preview = clean.length ? [] : parseAgencyInventoryPreview({
    text: normalizeBodyText(cheerio.load(fetched.text)("body").text()),
    source: "Grupo Q Usados",
    url,
    fx,
  });
...
async function searchQualityMotors(query, spec, fx) {
  ...
  const clean = dedupeCarOffers(rows)
    ...
    .slice(0, 25);
  const preview = clean.length ? [] : parseAgencyInventoryPreview({
    text: normalizeBodyText($("body").text()),
    source: "Kia / Quality Motors",
    url: inventoryUrl,
    fx,
  });
...
async function searchCarSources(query) {
  ...
  const allOffers = dedupeCarOffers(sources.flatMap((x) => x.offers));
  const yearAlternatives = spec.year
    ? allOffers
        .filter((offer) => offer.year && offer.year !== spec.year)
        .sort((a, b) => Math.abs(a.year - spec.year) - Math.abs(b.year - spec.year))
        .slice(0, 40)
    : [];
  const inventoryAlternatives = sources.flatMap((source) => source.alternatives || []);
  const alternatives = dedupeCarOffers([...yearAlternatives, ...inventoryAlternatives]).slice(0, 40);
...
async function searchGenericTech(config, query) {
  ...
  const candidates = [url, ...(config.fallbackUrls || [])];
  let fetched = await fetchText(candidates[0], 12000);
  let fetchedUrl = candidates[0];

  for (const candidate of candidates.slice(1)) {
    if (fetched.ok) break;
    fetched = await fetchText(candidate, 12000);
    fetchedUrl = candidate;
  }

  if (!fetched.ok) return sourceStatus(config.name, url, [], fetched.error);

  let offers = genericTechFromHtml(
    cheerio.load(fetched.text),
    config.name,
    fetchedUrl,
    query,
    config.minPrice
  ...
  if (!offers.length) {
    for (const candidate of candidates.slice(1)) {
      const fallback = await fetchText(candidate, 12000);
      if (!fallback.ok) continue;
      const fallbackOffers = genericTechFromHtml(
        cheerio.load(fallback.text),
        config.name,
        candidate,
        query,
        config.minPrice
  ...
  return sourceStatus(config.name, url, offers);
}
