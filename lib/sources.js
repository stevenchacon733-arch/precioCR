...
const SUPPLEMENT_CONFIG = [
  {
    ...
    minPrice: 1000,
    buildUrl: (q) =>
      `https://fitmart.cr/?s=${encodeURIComponent(q)}&post_type=product`,
  },
  {
    name: "Bionatural CR",
    minPrice: 1000,
    buildUrl: (q) =>
      `https://bionaturalcr.com/?s=${encodeURIComponent(q)}&post_type=product`,
  },
  {
    name: "Fitness Shop CR",
    minPrice: 1000,
    buildUrl: () => "https://fitnesshopcr.sumerlabs.com/",
  },
];

// Farmacias con catálogo en línea real en Costa Rica. Walmart y MaxiPalí
// comparten la misma plataforma VTEX (grupo Walmart Centroamérica), por lo
// que reutilizan el mismo tipo de conector que las fuentes de tecnología.
// Fischel y Sucre exponen HTML propio; Fischel carga los resultados por
// JavaScript, así que su conector es "mejor esfuerzo" (puede devolver 0
// resultados según el momento, pero nunca falla la búsqueda completa).
const MEDICATION_VTEX_CONFIG = [
  {
    name: "MaxiPalí",
    domain: "maxipali.co.cr",
    minPrice: 200,
  },
];
...
const MEDICATION_CONFIG = [
  {
    ...
    minPrice: 200,
    buildUrl: (q) =>
      `https://sucreenlinea.com/catalogsearch/result/?q=${encodeURIComponent(q)}`,
  },
  {
    name: "Fischel en línea",
    minPrice: 200,
    buildUrl: (q) => `https://fischelenlinea.com/busqueda?f=${encodeURIComponent(q)}`,
  },
];
...
const EXTERNAL_SEARCHES = {
  ...
  medication: [
    {
      name: "FarmaValue",
      note: "No tiene catálogo web público; solo app propia o WhatsApp",
      buildUrl: (q) =>
        `https://www.google.com/search?q=${encodeURIComponent("FarmaValue Costa Rica precio " + q)}`,
    },
    {
      name: "Farmacia del Norte",
      note: "Farmacia local sin tienda en línea pública",
      buildUrl: (q) =>
        `https://www.google.com/search?q=${encodeURIComponent("Farmacia del Norte Costa Rica precio " + q)}`,
    },
  ],
};
...
async function searchVtexStore(name, domain, query, minPrice) {
  const q = query.trim();
  const apiUrl =
    `https://${domain}/api/catalog_system/pub/products/search/` +
    `${encodeURIComponent(q)}?_from=0&_to=49`;
  const publicUrl = `https://${domain}/${encodeURIComponent(q)}?_q=${encodeURIComponent(q)}&map=ft`;

  const fetched = await fetchJson(apiUrl);
  if (!fetched.ok || !Array.isArray(fetched.data)) {
    return sourceStatus(name, publicUrl, [], fetched.error || "Catálogo no disponible");
  }
  ...
  for (const product of fetched.data) {
    const productName = cleanTitle(product?.productName || "");
    if (!productName || relevance(productName, q) < 0.45) continue;
    ...
    for (const item of product?.items || []) {
      for (const seller of item?.sellers || []) {
        ...
        if (!Number.isFinite(price) || price < minPrice || available <= 0) continue;
        ...
        offers.push({
          ...
          url:
            product?.link ||
            (product?.linkText ? `https://${domain}/${product.linkText}/p` : publicUrl),
          condition: "Nuevo",
          meta: "",
        });
      }
  ...
  return sourceStatus(name, publicUrl, dedupeGeneric(offers).slice(0, 20));
}

async function searchMaxiPali(query) {
  return searchVtexStore("MaxiPalí", "www.maxipali.co.cr", query, 200);
}

async function searchMaxiPaliCatalogSafe(query, type) {
  const result = await searchMaxiPali(query);

  const offers = (result.offers || []).filter((offer) =>
    isAllowedCatalogOffer(`${offer.title} ${offer.meta || ""}`, type)
  );
  ...
  return {
    ...
    count: offers.length,
    offers,
    error: offers.length ? null : "Sin coincidencias permitidas",
  };
}

async function searchMedicationSources(query) {
  const jobs = [
    searchWalmartCatalogSafe(query, "medication"),
    searchMaxiPaliCatalogSafe(query, "medication"),
    ...MEDICATION_CONFIG.map((config) =>
      searchGenericCatalog(config, query, "medication")
    ),
  ];
  ...
  const names = [
    "Walmart Costa Rica",
    "MaxiPalí",
    ...MEDICATION_CONFIG.map((c) => c.name),
  ];
  ...
  const offers = dedupeGeneric(
    sources.flatMap((source) => source.offers || [])
  )
    .filter((offer) => isAllowedCatalogOffer(offer.title, "medication"))
    .sort((a, b) => a.price - b.price)
    .slice(0, 80);

  const externalSources = EXTERNAL_SEARCHES.medication.map((source) => ({
    name: source.name,
    url: source.buildUrl(query),
    note: source.note,
  }));
...
async function searchWalmartCatalogSafe(query, type) {
  const result = await searchWalmart(query);
...
async function searchGenericCatalog(config, query, type) {
  const url = config.buildUrl(query);
  const fetched = await fetchText(url);

  if (!fetched.ok) {
    return sourceStatus(config.name, url, [], fetched.error);
  }

  const $ = cheerio.load(fetched.text);
  const offers = genericTechFromHtml(
    $,
    config.name,
    url,
    query,
  ...
  return sourceStatus(
    ...
    url,
    offers,
    offers.length ? null : "Sin coincidencias permitidas"
  );
}

async function searchSupplementSources(query) {
  const jobs = [
    searchWalmartCatalogSafe(query, "supplement"),
    ...SUPPLEMENT_CONFIG.map((config) =>
      searchGenericCatalog(config, query, "supplement")
    ),
  ];
  ...
  const offers = dedupeGeneric(
    ...
    .filter((offer) => isAllowedCatalogOffer(offer.title, "supplement"))
    .sort((a, b) => a.price - b.price)
    .slice(0, 80);
...
export async function searchSources(query, type) {
  ...
  if (type === "supplement") return searchSupplementSources(query);
  if (type === "medication") return searchMedicationSources(query);
