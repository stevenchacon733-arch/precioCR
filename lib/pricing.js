...
export function median(values) {
  const valuesSorted = sortedNumbers(values);
  return percentile(valuesSorted, 0.5);
}
...
function removePriceOutliers(offers) {
  ...
  // Used cars can legitimately vary more than commodity products.
...
function sourceBalancedMedian(offers) {
  ...
  for (const offer of offers) {
    ...
    bySource.get(offer.source).push(offer.price);
  }

  const sourceMedians = [...bySource.entries()].map(([source, prices]) => ({
    source,
...
function groupBreakdown(offers) {
  ...
  return groups.map((key) => {
    ...
    return {
      ...
      count: rows.length,
      median: median(rows.map((x) => x.price)),
...
export function analyzeCarMarket(offers, spec) {
  ...
  const groupCount = new Set(clean.map((x) => x.sourceGroup).filter(Boolean)).size;
  ...
  const portal = breakdown.find((x) => x.key === "portal");
  const agencia = breakdown.find((x) => x.key === "agencia");
  ...
  return {
    ...
    average: clean.length
      ? Math.round(clean.reduce((sum, x) => sum + x.price, 0) / clean.length)
      : null,
    buyTarget,
    sellRecommended,
    score,
    ...
    particularMedian: particular?.median || null,
    portalMedian: portal?.median || null,
    agencyMedian: agencia?.median || null,
  };
}

export function summarizePrices(offers) {
  const prices = sortedNumbers(offers.map((o) => o.price));
